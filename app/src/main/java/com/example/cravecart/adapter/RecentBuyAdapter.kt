...
import com.example.cravecart.databinding.RecentbuyitemBinding
...
holder.bind(position)    }
   inner class RecentViewHolder(private val binding: RecentbuyitemBinding):RecyclerView.ViewHolder(binding.root) {
        fun bind(position: Int) {
            binding.apply {
                RecentBuyItemName.text= foodNameList[position]
                RecentBuyItemPrice.text=foodPriceList[position]
                RecentBuyItemQuantity.text=foodQuantityList[position].toString()
                val uriString=foodImageList[position]
                val uri=Uri.parse(uriString)
                Glide.with(context).load(uri).into(RecentBuyItemImage)
            }
        }
